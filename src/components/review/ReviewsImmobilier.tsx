import {
  ...
  createReview,
  getLastFiveRatingsForImmobilier,
  getAverageRatingForImmobilier,
...
export default function ReviewsImmobilier({
  immobilierId,
}: {
  immobilierId: string;
}) {
  ...
  useEffect(() => {
    async function fetchReviews() {
      const ratings = await getLastFiveRatingsForImmobilier(immobilierId);
      const avgRating = await getAverageRatingForImmobilier(immobilierId);
      setReviews(ratings);
  ...
  async function handleAddReview() {
    ...
    await createReview({
      ...reviewData,
      vehicleId: null,
      immobilierId, // A remplacer par l'id du bien immobilier si disponible
      id: uuid(),
      createdAt: new Date(),
    });
    ...
    // Rafraîchir les avis
    const updatedRatings = await getLastFiveRatingsForImmobilier(immobilierId);
    const updateAvgRating = await getAverageRatingForImmobilier(immobilierId);
